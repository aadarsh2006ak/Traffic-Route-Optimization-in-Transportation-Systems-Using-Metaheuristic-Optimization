...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        page = st.radio("Navigation", 
            ["Route Optimizer", "Quantum Analytics", "Benchmark Lab", "Network Graph"], 
        ...
        with st.expander("⚙️ Optimization Mode", expanded=True):
            algo = st.selectbox("Algorithm", ["QPSO", "Simulated Annealing", "Genetic Algorithm", "Ant Colony"], 
                                 help="Quantum-inspired vs classical metaheuristics")
            st.session_state.algorithm_choice = algo
            
            fleet_size = st.slider("Fleet Size", 1, 4, 1)
            capacity = st.number_input("Vehicle Capacity (stops/vehicle, 0=unlimited)", min_value=0, value=0)
        ...
        # --- Traffic Simulation ---
        with st.expander("🚦 Traffic Simulation", expanded=False):
            traffic_on = st.toggle("Enable Traffic Congestion", value=False)
            st.session_state.traffic_enabled = traffic_on
            if traffic_on:
                hour = st.slider("Time of Day", 0, 23, 9, help="Peak hours (8-10, 17-19) = higher congestion")
                st.session_state.traffic_hour = hour

        # --- Group 3: Advanced Parameters ---
        with st.expander("⚛️ Advanced Parameters", expanded=False):
            if algo == "QPSO":
                swarm_size = st.slider("Swarm Size", 10, 200, 50, help="Number of quantum particles")
                beta = st.slider("Contraction-Expansion Coeff (β)", 0.5, 1.5, 1.0, format="%.2f", help="Controls exploration/exploitation")
                max_iter = st.slider("Max Iterations", 100, 3000, 1000, help="Maximum quantum iterations")
                ...
                pop_size = st.slider("Population Size", 20, 200, 50, help="Number of candidate solutions per generation")
                generations = st.slider("Generations", 50, 2000, 500, help="Number of evolutionary iterations")
                mutation_rate = st.slider("Mutation Rate", 0.01, 0.50, 0.10, format="%.2f", help="Probability of genetic mutation")
                crossover_rate = st.slider("Crossover Rate", 0.50, 1.00, 0.80, format="%.2f", help="Probability of chromosome crossover")
                q_params = {"pop_size": pop_size, "generations": generations, "mutation_rate": mutation_rate, "crossover_rate": crossover_rate}
            elif algo == "Ant Colony":
                num_ants = st.slider("Number of Ants", 5, 100, 20, help="Ant agents exploring routes")
                iterations = st.slider("Iterations", 50, 1000, 200, help="Number of foraging cycles")
                alpha = st.slider("Pheromone Importance (α)", 0.1, 5.0, 1.0, format="%.2f", help="Weight of pheromone trail")
                beta_aco = st.slider("Heuristic Importance (β)", 0.1, 5.0, 2.0, format="%.2f", help="Weight of distance heuristic")
                evap_rate = st.slider("Evaporation Rate (ρ)", 0.01, 0.99, 0.10, format="%.2f", help="Pheromone decay rate per cycle")
        ...
        if show_routes:
            for idx, route_geo in enumerate(d['routes_geo']):
                if st.session_state.get('traffic_enabled', False):
                    # Congestion factor from backend or calculated from simulation hour
                    congestion_factor = st.session_state.get('congestion_factor')
                    if congestion_factor is None:
                        hour = st.session_state.get('traffic_hour', 9)
                        congestion_factor = 1.6 if (8 <= hour <= 10 or 17 <= hour <= 19) else 1.3 if (11 <= hour <= 16) else 1.0
                    color = "#ff2b2b" if congestion_factor > 1.5 else "#ff9100" if congestion_factor > 1.2 else "#00e5ff"
                else:
...
def render_benchmark_view():
    """Benchmark Lab - compares QPSO vs classical algorithms on same problem."""
    st.markdown("### 🏁 Algorithm Benchmark Lab")
    st.caption("Run all algorithms on the same route problem and compare.")
    
    algos_to_test = st.multiselect(
        "Select algorithms to compare",
        ["QPSO", "Simulated Annealing", "Genetic Algorithm", "Ant Colony", "Exact (small instances only)"],
        default=["QPSO", "Simulated Annealing", "Genetic Algorithm"]
    )
    
    if st.button("▶️ Run Benchmark", type="primary"):
        if not st.session_state.stops_data:
            st.error("Load stops first from Route Optimizer page.")
        else:
            st.session_state.benchmark_running = True
            # backend call yahan hoga (logic.py se benchmark_runner function)
    
    if st.session_state.benchmark_results:
        results = st.session_state.benchmark_results
        # Table
        st.markdown("#### Results Summary")
        st.dataframe(results['summary_table'])  # columns: Algorithm, Distance(km), Time(s), Iterations, Gap from Best(%)
        
        # Multi-line convergence chart
        st.markdown("#### Convergence Comparison")
        st.line_chart(results['convergence_df'])  # x=iteration, one column per algorithm
        
        # Bar chart: runtime comparison
        st.markdown("#### Runtime Comparison")
        st.bar_chart(results['runtime_df'])

def render_graph_view():
    """Shows the abstract weighted graph model, separate from the road map."""
    st.markdown("### 🕸️ Network Graph Model")
    st.caption("Nodes = locations, Edges = weighted distance/time/congestion")
    
    if st.session_state.stops_data:
        stops = st.session_state.stops_data
        
        # Build DOT string for graphviz
        dot_lines = [
            'digraph G {',
            '    rankdir=LR;',
            '    bgcolor="transparent";',
            '    node [shape=box, style="filled,rounded", fillcolor="#1a1f2c", fontcolor="#ffffff", color="#00e5ff", fontname="Roboto"];',
            '    edge [color="#4a5568", fontcolor="#a0aab5", fontname="Roboto", fontsize=10];'
        ]
        
        # Add Nodes
        for i, stop in enumerate(stops):
            label = stop['name'].split(',')[0].replace('"', '')
            dot_lines.append(f'    node_{i} [label="{i+1}. {label}"];')
        
        # If optimized route exists, highlight the route edges
        if st.session_state.optimized_route and 'markers' in st.session_state.optimized_route:
            markers = st.session_state.optimized_route['markers']
            vehicles = {}
            for m in markers:
                v_id = m.get('vehicle_id', 0)
                vehicles.setdefault(v_id, []).append(m)
            
            colors = ["#00e5ff", "#00e676", "#ff9100", "#d500f9"]
            for v_id, v_markers in vehicles.items():
                v_color = colors[v_id % len(colors)]
                for k in range(len(v_markers) - 1):
                    src_name = v_markers[k]['name'].split(',')[0]
                    dst_name = v_markers[k+1]['name'].split(',')[0]
                    
                    src_idx = next((idx for idx, s in enumerate(stops) if s['name'].split(',')[0] == src_name), None)
                    dst_idx = next((idx for idx, s in enumerate(stops) if s['name'].split(',')[0] == dst_name), None)
                    
                    if src_idx is not None and dst_idx is not None:
                        dot_lines.append(f'    node_{src_idx} -> node_{dst_idx} [color="{v_color}", penwidth=2.5, label="V{v_id+1} Leg {k+1}"];')
        else:
            # Connect sequentially or as a demo mesh
            max_edges = min(len(stops), 10)
            for i in range(max_edges - 1):
                dot_lines.append(f'    node_{i} -> node_{i+1} [label="edge", style=dashed];')
        ...
        st.info("Load stops first from Route Optimizer page.")
